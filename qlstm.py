# sleep_train.py
# HMC Sleep Staging — Hybrid CNN→QNN model with robust pathing + quiet logs + progress bars
...
# ---------------------------
# Silence MNE chatter & common warnings
# ---------------------------
warnings.filterwarnings("ignore", category=RuntimeWarning)
warnings.filterwarnings("ignore", category=UserWarning, module="mne")
mne.set_log_level("ERROR")  # suppress MNE info like "Reading 0 ... secs"
...
EPOCH_SEC = 30
VERBOSE_LOAD = False  # keep console clean; set True for per-file logs
...
class HMCSleepDataset(Dataset):
    def __init__(self, root: Path, preferred_channels: Optional[List[str]] = None, verbose: bool = True):
        ...
        # Use Path.glob and support .edf / .EDF, filter out scoring files
        edfs = sorted(
...
# ---------------------------
# Classical models (kept for reference; unused by default)
# ---------------------------
class SmallEEGCNN(nn.Module):
    def __init__(self, n_ch=4, n_cls=5):
        ...
        self.net = nn.Sequential(
            ...
            nn.MaxPool1d(4),  # 3000 -> 750 at 100Hz*30s = 3000
...
class HybridCNN_QNN(nn.Module):
    """
    CNN feature extractor -> (Linear -> n_qubits) -> Quantum layer -> Linear(n_qubits -> n_cls)
    Notes for performance:
      - AngleEmbedding of compact features is robust on EEG.
      - StronglyEntanglingLayers (3 blocks) balance expressivity & trainability.
      - Expectation outputs (Z on each wire) feed a small linear head.
    """
    def __init__(
        self,
        n_ch: int = 4,
        ...
        n_layers: int = 3,
        feat_width: int = 128,
        shots: Optional[int] = None,  # None => analytic; set to e.g. 2048 for shot noise
    ):
        super().__init__()
        self.n_qubits = n_qubits
        self.n_layers = n_layers

        # --- 1) Strong CNN encoder (robust for EEG)
        self.feat = nn.Sequential(
            nn.Conv1d(n_ch, 32,  kernel_size=7, padding=3), nn.BatchNorm1d(32), nn.ReLU(),
            nn.Conv1d(32, 64,   kernel_size=7, padding=3),  nn.BatchNorm1d(64), nn.ReLU(),
            nn.MaxPool1d(4),   # 3000 -> 750
        ...
        # --- 2) Classical bottleneck to match qubits
        self.to_angles = nn.Sequential(
            nn.Linear(feat_width, n_qubits, bias=True)
        )

        # --- 3) Quantum layer (PennyLane QNode -> TorchLayer)
        self.dev = qml.device("default.qubit", wires=n_qubits, shots=shots)

        @qml.qnode(self.dev, interface="torch", diff_method="backprop")
        def qnode(inputs, weights):
            # inputs: shape [n_qubits]
            # weights: StronglyEntanglingLayers params, shape (n_layers, n_qubits, 3)
            qml.AngleEmbedding(inputs, wires=range(n_qubits), rotation="Y")
            qml.StronglyEntanglingLayers(weights, wires=range(n_qubits))
            # Return Z expectation of each wire
            return [qml.expval(qml.PauliZ(i)) for i in range(n_qubits)]

        weight_shapes = {"weights": (n_layers, n_qubits, 3)}
        self.q_layer = qml.qnn.TorchLayer(qnode, weight_shapes)

        # --- 4) Small linear head: n_qubits -> classes
        self.head = nn.Sequential(
            nn.Linear(n_qubits, 64), nn.ReLU(), nn.Dropout(0.2),
            nn.Linear(64, n_cls),
        )

        # Good inits help stability
    ...
    def forward(self, x):
        ...
        angles = self.to_angles(f)       # [B, n_qubits]
...
def run():
    ...
    val_loader   = DataLoader(val_ds,   batch_size=64, shuffle=False, num_workers=0, drop_last=False)
    ...
    model = HybridCNN_QNN(
        n_ch=n_ch, n_cls=5,
        n_qubits=8,       # try 8 first; can bump to 12 if stable
        n_layers=3,
        feat_width=128,
        shots=None        # set to e.g. 2048 to simulate hardware noise (slower)
    ).to(device)

    # Label smoothing + class weights can stabilize hybrid training
    criterion = nn.CrossEntropyLoss(
        weight=torch.tensor(cw, device=device, dtype=torch.float32),
        label_smoothing=0.05
    )

    # Slightly lower LR often helps QNN layers; use cosine decay
    base_lr = 2e-4
    ...
    def run_epoch(loader, train=True, epoch=0):
        ...
        for xb, yb in bar:
            ...
            if train:
                loss.backward()
                # Grad clip is very helpful for hybrid models
    ...
    for epoch in range(EPOCHS):
        ...
        print(f"Epoch {epoch+1:02d} | train {tr_acc:.3f} loss {tr_loss:.3f} | val {va_acc:.3f} loss {va_loss:.3f}")
