# sleep_train.py
# HMC Sleep Staging — CNN-BiLSTM baseline with robust pathing + quiet logs + progress bars
...
def run():
    ...
    model = CNN_BiLSTM(n_ch=n_ch, n_cls=5, hidden=128, bidirectional=True).to(device)
    # model = SmallEEGCNN(n_ch=n_ch, n_cls=5).to(device)

    criterion = nn.CrossEntropyLoss(weight=torch.tensor(cw, device=device, dtype=torch.float32))
    ...
    for epoch in range(EPOCHS):
        ...
        if va_acc > best_val:
            best_val = va_acc
            torch.save(model.state_dict(), "hmc_cnn_best.pt")
